...
public:
    ...
    inline void set_all(void) {
        memset(bits,0xFF,nentries*BIT_ENTRY_SIZE);
        //ensure that the "extra" bits on the last entry are all zero
        BIT_ENTRY bit=n&MASK;
        BIT_ENTRY entry=~(((BIT_ENTRY) 1)<<bit); //=0x000FFFFF for example
        bits[nentries-1]=entry;
    }
    //copy constructor
