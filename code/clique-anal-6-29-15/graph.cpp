...
bool graph::is_edge(long int i, long int j)
{
    ...
    return is_set(incidence,index(i,j));
}
...
void graph::add_edge(long int i, long int j)
{
    ...
    if (is_set(incidence,idx)) return; //don't add edge twice
    set_bit(incidence,idx);
...
void graph::remove_edge(long int i, long int j)
{
    ...
    long int idx = index(i,j);
    if (!is_set(incidence,idx)) return; //don't remove edge twice
    clear_bit(incidence,idx);
...
//depth first search
void graph::id_connected_component(long int ivertex, long int component, long int * ids)
{
    long int j,jvertex;
...
long int graph::connected_components(long int * ids)
{
    long int ivertex,component;
...
void to_clique_info(long int n, char * bits, clique_info * info)
{
    int i;
    info->count=0;
    info->vertices=NULL;
    for (i=0; i<n; i++) if (is_set(bits,i)) {
...
void graph::degeneracy_ordering(vector<long int> * order, long int * degen)
{
    ...
    vsets=(char * *) checkalloc(max_d+1,sizeof(char *));
    for (i=0; i<=max_d; i++) vsets[i]=new_empty(nvertices);
...
void graph::detect_cliques(int call_level, vector<clique_info> * cliques, char * r, char * p, char * x)
{
    ...
    for (v=0; v<nvertices; v++) if (is_set(p,v) && ((pivot==v) || !is_edge(pivot,v))) {
        ...
        print_set("R",nvertices,newr);
        print_set("P",nvertices,newp);
        print_set("X",nvertices,newx);*/
        //recursive call
